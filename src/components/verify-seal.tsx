...
/** ตราไอคอนยืนยันสีเขียว + ข้อความ เช่น "ผ่านการให้ยาแล้ว" */
export function SealLabel({ children }: { children: string }) {
  return (
    <span className="inline-flex items-center gap-1">
      <VerifySeal passed label={children} className="h-[18px] w-[18px]" />
      <span className="text-xs font-medium text-emerald-700 dark:text-emerald-400">{children}</span>
    </span>
  );
}
